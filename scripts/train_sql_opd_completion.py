#!/usr/bin/env python3
"""Continue-train an SFT SQL adapter on OPD teacher completion data."""
...
def main() -> None:
    ...
    parser.add_argument("--student-adapter-path", required=True)
    ...
    eval_dataset = CompletionDataset(eval_rows, tokenizer, args.max_length)
    print(
        json.dumps(
            {
    ...
    model = PeftModel.from_pretrained(base, args.student_adapter_path, is_trainable=True)
